...
const AdminPathwayEdit = () => {
  ...
  const addStep = () => {
    setSteps([...steps, { title: "", description: "", type: "course", resource: "" }]);
  };
  ...
  const handleUpdatePathway = async () => {
    ...
    const formattedSteps = steps.map((step, index) => {
      ...
      return { ...step, order: index + 1 }; // Ensure order is exactly right
    });
      ...
      <div className="mx-auto max-w-4xl space-y-6 pb-12">
        ...
        <div className="rounded-[28px] border border-black/5 bg-white/70 p-6 shadow-sm backdrop-blur">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1 block text-sm font-medium text-text-dark">Pathway Name</label>
              <input
                type="text"
                name="pathName"
            ...
            <div>
              <label className="mb-1 block text-sm font-medium text-text-dark">Level</label>
              <select
                name="level"
                value={pathway.level}
                onChange={handlePathwayChange}
        ...
        <div className="space-y-4">
          ...
          {steps.map((step, index) => (
            <div key={index} className="relative rounded-[22px] border border-black/5 bg-white/80 p-5 shadow-sm">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="sm:col-span-2">
                  <label className="mb-1 block text-xs font-medium text-text-dark">Step Title</label>
                  <input
                    type="text"
                    value={step.title}
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-text-dark">Content Type</label>
                  <select
                    value={step.type}
                ...
                <div>
                  <label className="mb-1 block text-xs font-medium text-text-dark">Resource URL</label>
                  <input
                    type="text"
                    value={step.resource || ""}
