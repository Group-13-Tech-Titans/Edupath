        ...
        <div className="flex items-center justify-between mb-8 bg-white p-6 rounded-3xl shadow-sm border border-emerald-100/50">
          <div>
            <h1 className="text-2xl font-bold text-slate-800">
              Select a Platform Course
            </h1>
            <p className="text-slate-500 text-sm mt-1">
              Browse and attach existing EduPath courses to your pathway step.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {loadingCourses ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredCourses.length === 0 ? (
              <p className="col-span-full text-center text-slate-500 py-10 italic">
              ...
              filteredCourses.map((course) => (
                <div
                  ...
                  className="bg-white rounded-[24px] shadow-sm border border-slate-100 overflow-hidden flex flex-col hover:shadow-xl hover:-translate-y-1 hover:border-primary/40 transition-all duration-300"
                >
                  <img
                    src={
                      course.thumbnailUrl ||
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      <span
                        className={`text-xs font-bold px-3 py-1.5 rounded-full ${course.status === "pending" ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                      >
                        {course.status
...
const AdminPathwayEdit = () => {
  ...
  const handleFileUpload = async (event, stepIndex, resIndex) => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:5000/api/upload/content",
        formData,
        {
          headers: {
      ...
      if (res.data.success) {
        ...
        // 🟢 STRICT: Force type to "pdf" for uploaded documents
  ...
  const handleAttachCourse = (course) => {
    ...
    const exists = newSteps[stepIdx].linkedCourses.find(
  ...
  const handleUpdatePathway = async () => {
    ...
    const formattedSteps = steps.map((step, index) => {
      ...
      // 🟢 2. NEW: Validate Learning Materials (Links & PDFs)
            ...
            return (
              ...
              >
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  {/* 🟢 UPDATED: EXTERNAL LINKS & UPLOAD UI */}
                      ...
                      {(step.resources || []).map((res, resIndex) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* 🟢 STRICT CONDITIONAL: PDF gets Upload Button, others get URL input */}
                              ...
                              {res.type === "pdf" ? (
                                <>
                                  <input
                                    ...
                                    value={res.url}
                                    readOnly // Make it read-only so they are forced to upload
