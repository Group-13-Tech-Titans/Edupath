...
import { trackCourseView } from "../../api/subscriptionApi.js";
...
const StudentCourseDetail = () => {
  ...
  const handleEnrollClick = async () => {
    ...
    if (res.success) {
      toast.success("Successfully enrolled! You can now watch the course.");
  ...
  useEffect(() => {
    const fetchCourseAndStatus = async () => {
      try {
        ...
        try {
          const singleRes = await axios.get(`${API_BASE_URL}/courses/${id}`, config);
          foundCourse = singleRes.data.course || singleRes.data;
        } catch (singleErr) {
          const { data: courseData } = await axios.get(`${API_BASE_URL}/courses`, config);
          const allCourses = courseData.courses || courseData || [];
          foundCourse = allCourses.find((c) => String(c._id || c.id) === String(id));
        }
        ...
        if (token) {
          const { data: userData } = await axios.get(`${API_BASE_URL}/auth/me`, config).catch(() => ({ data: {} }));
          const freshUser = userData.user || userData || currentUser;
          
          const isUserEnrolled = freshUser?.enrolledCourses?.some(
            (c) => String(c.courseId) === String(id) || (foundCourse && String(c.courseId) === String(foundCourse._id))
          );
          
          setLocalIsEnrolled(Boolean(isUserEnrolled));
        } else {
