...
const StudentPathway = () => {
  ...
  if (!pathway) {
    return (
      <PageShell>
        <div className="mx-auto max-w-4xl pt-10 text-center px-4">
          <div className="bg-slate-800 rounded-[32px] p-8 sm:p-10 text-white shadow-xl mb-10 text-left relative overflow-hidden flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="max-w-md">
              <span className="inline-block bg-primary/20 text-primary text-xs font-black uppercase tracking-wider px-3 py-1 rounded-full mb-3 border border-primary/30">
                Personalized Learning Map
              </span>
              <h2 className="text-2xl sm:text-3xl font-black mb-2">Create Your Custom Pathway</h2>
              <p className="text-slate-300 text-sm">
                Answer a couple of quick questions to discover your personalized milestone journey, resources, and quizzes.
              </p>
              <button
                onClick={() => navigate("/student/path-finder")}
                className="mt-6 bg-primary hover:brightness-110 text-white font-black px-7 py-3 rounded-full text-xs shadow-lg shadow-primary/25 transition-all hover:scale-105 active:scale-95 cursor-pointer"
              >
                LAUNCH PATH FINDER ✨
              </button>
            </div>
            <div className="text-7xl shrink-0 hidden md:block">🧭</div>
          </div>

          {availableTemplates.length > 0 && (
            <>
              <h3 className="text-xl font-black text-slate-800 mb-6 text-left">Or Explore Published Pathways</h3>
              <div className="grid gap-6 sm:grid-cols-2 text-left">
                {availableTemplates.map((t) => (
                  <motion.div whileHover={{ y: -4 }} key={t._id} className="p-6 bg-white rounded-3xl shadow-lg border border-slate-100 flex flex-col justify-between">
                    <div>
                      <span className="bg-slate-100 text-slate-700 text-[10px] font-black uppercase px-2.5 py-0.5 rounded-full mb-2 inline-block">
                        {t.level || "Beginner"} Level
                      </span>
                      <h4 className="text-lg font-bold text-slate-800 mb-1">{getPathwayName(t.pathName)}</h4>
                      <p className="text-slate-500 text-xs mb-4">{t.steps?.length || 0} Milestones Included</p>
                    </div>
                    <button onClick={() => handleEnroll(t._id)} className="bg-slate-800 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-full font-bold text-xs w-full transition-all cursor-pointer">
