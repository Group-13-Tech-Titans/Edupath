  ...
  useEffect(() => {
    const loadSubscription = () => {
      getSubscriptionStatus()
        ...
        .catch((err) => console.error("Failed to load subscription on profile:", err));
    };

    loadSubscription();
    window.addEventListener("edupath_subscription_updated", loadSubscription);
    return () => window.removeEventListener("edupath_subscription_updated", loadSubscription);
  }, []);
            ...
            <div className="lg:col-span-8">
              <div className="rounded-[2.5rem] bg-white/70 p-8 shadow-xl shadow-emerald-900/5 border border-white backdrop-blur-md">
                ...
                <form onSubmit={handleSave} className="space-y-10">
                  ...
                  <section>
                    <div className="rounded-[1.5rem] border border-black/5 bg-white p-6 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                      <div>
                        <div className="flex flex-wrap items-center gap-3 mb-1.5">
                          <h4 className="font-bold text-slate-800 text-[14px] leading-tight">Membership & Subscription Plan</h4>
                          <span className={`shrink-0 whitespace-nowrap rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${
                            isPremium
                        ...
                        <p className="text-[12px] leading-relaxed text-[#6B7280]">
                          {isPremium
                            ? "Unlimited course watching & up to 20 active pathways."
                            : "Up to 10 courses per month & 3 lifetime pathways."}
                        </p>
                      </div>

                      <a
                        href="/student/plans"
                        className="shrink-0 self-start sm:self-auto bg-[#0f172a] hover:bg-[#1e293b] text-white text-[13px] font-semibold px-6 py-3 rounded-full transition-all flex items-center gap-2"
                      >
                        <span>{isPremium ? "Manage Subscription" : "Upgrade to Premium ($49/mo)"}</span>
                        <span>→</span>
                      </a>
                    </div>
                  </section>
