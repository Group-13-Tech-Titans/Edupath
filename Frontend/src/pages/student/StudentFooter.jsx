...
const StudentFooter = () => {
  return (
    <footer className="mt-auto rounded-t-2xl bg-white px-7 py-8 shadow-[0_-4px_20px_rgba(0,0,0,0.04)]">
      <div className="mx-auto max-w-6xl">
          ...
          <div className="grid grid-cols-1 gap-8 sm:grid-cols-3 md:gap-12">
            <FooterCol
              ...
              links={[
                ...
                { label: "Mentorship", href: "/student/mentor" },
        ...
        <div className="mt-8 border-t-2 border-slate-100 pt-5 text-center">
          <p className="text-sm text-slate-500">© {new Date().getFullYear()} EduPath. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
...
function SocialIcon({ children }) {
  return (
    <button
      type="button"
      className="grid h-10 w-10 place-items-center rounded-full bg-slate-50 text-slate-400 transition hover:bg-primary/10 hover:text-primary"
    >
      {children}
    </button>
  );
...
function EduPathLogo() {
  return (
    <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/10 text-primary shadow-sm text-sm">
      🎓
    </div>
  );
}
