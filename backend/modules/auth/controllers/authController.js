...
/**
 * Returns a client-safe user object with password stripped and
 * a normalized subscription (includes isPremium) so profiles stay in sync.
 */
async function toSafeUser(user) {
  let subscriptionStatus = null;
  if (user.role === "student") {
    try {
      subscriptionStatus = await subscriptionService.normalizeUserSubscription(user);
    } catch (err) {
      console.error("Failed to normalize subscription for user payload:", err.message);
    }
  }
  ...
  if (safe._id) safe.id = safe._id.toString();

  const storedSub = safe.subscription && typeof safe.subscription === "object" ? safe.subscription : {};
  const plan = subscriptionStatus?.plan || storedSub.plan || "free";
  safe.subscription = {
    ...storedSub,
    plan,
    isPremium: subscriptionStatus?.isPremium ?? plan === "premium",
    billingCycle: subscriptionStatus?.billingCycle || storedSub.billingCycle || "monthly",
    status: subscriptionStatus?.status || storedSub.status || "active",
    startDate: subscriptionStatus?.startDate || storedSub.startDate || null,
    currentPeriodEnd: subscriptionStatus?.currentPeriodEnd || storedSub.currentPeriodEnd || null,
  };
...
exports.login = async (req, res) => {
  try {
    ...
    res.json({ token, user: await toSafeUser(user) });
  } catch (err) {
...
exports.getCurrentUser = async (req, res) => {
  try {
    // Re-load so subscription is current, then attach isPremium for profile UIs
