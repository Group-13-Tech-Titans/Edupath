...
// EDUCATORS
router.get("/educators/pending", authMiddleware, roleMiddleware(["admin"]), adminController.getPendingEducators); //show pending educators 

router.patch("/educators/:id/verify", authMiddleware, roleMiddleware(["admin"]), adminController.verifyEducator); //mark verified educator

router.patch("/educators/:id/reject", authMiddleware, roleMiddleware(["admin"]), adminController.rejectEducator); //mark rejected educator

// COURSES
router.get("/courses/pending", authMiddleware, roleMiddleware(["admin"]), adminController.getPendingCourses);
//show all pending courses 

router.get("/courses/stats", authMiddleware, roleMiddleware(["admin"]), adminController.getCourseStats);
// show all courses count  ( pending, approved, rejected)
