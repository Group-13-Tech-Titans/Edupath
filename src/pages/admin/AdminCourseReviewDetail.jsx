...
const AdminCourseReviewDetail = () => {
  ...
  const handleSubmit = () => {
    ...
    const courses = JSON.parse(localStorage.getItem('all_courses') || JSON.stringify(mockCourses));
    const updatedCourses = courses.map(c => 
      c.id === id ? { ...c, status: reviewData.decision === 'approve' ? 'approved' : reviewData.decision === 'reject' ? 'rejected' : 'pending' } : c
    );
    localStorage.setItem('all_courses', JSON.stringify(updatedCourses));

    setShowToast(true);
    setTimeout(() => {
      navigate('/admin/course-review-queue');
    }, 1500);
  };
  ...
  return (
    <PageShell>
      <button onClick={() => navigate('/admin/course-review-queue')} className="btn btn-ghost btn-sm mb-md">← Back to Queue</button>
